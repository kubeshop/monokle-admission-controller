...
import {AdmissionRequest, AdmissionResponse, Violation} from "./admission.models";
import KubeClient from "../utils/kube-client";
import {Resource, ValidationResult} from "@monokle/validation";
import {ConfigService} from "@nestjs/config";
import {ValidationService} from "../policies/validation.service";
...
export class AdmissionController {
    ...
    constructor(private readonly $config: ConfigService, private readonly $validation: ValidationService) {
        this.ignoredNamespaces = this.$config.get<string[]>('ignoredNamespaces') ?? [];
    }
    ...
    async validate(@Body() body: AdmissionRequest): Promise<AdmissionResponse> {
        ...
        const validators = this.$validation.getMatchingValidators(resource, namespaceObject);
        ...
        const resourceForValidation = AdmissionController.createResourceForValidation(body);
            ...
            for (const result of validationResponse.result.runs) {
                for (const item of result.results) {
                    violations.push({
                        ...
                        level: item.level,
                        actions: actions,
                        name: AdmissionController.getFullyQualifiedName(item) ?? resourceForValidation.name
                    });
                }
            }
        }
        ...
        const responseFull = AdmissionController.handleViolationsByAction(violationsByAction, resourceForValidation, response);

        this.log.debug({response});
        return responseFull;
    }

    private static createResourceForValidation(admissionResource: AdmissionRequest): Resource {
        const resource = {
    ...
    private static handleViolationsByAction(violationsByAction: Record<string, Violation[]>, resource: Resource, response: AdmissionResponse) {
        for (const action of Object.keys(violationsByAction)) {
            ...
            if (action.toLowerCase() === 'warn') {
                response = this.handleViolationsAsWarn(violationsByAction[action], resource, response);
            } else if (action.toLowerCase() === 'deny') {
                const violationMessages = this.getViolationsMessages(violationsByAction[action], resource);
    ...
    private static handleViolationsAsWarn(violations: Violation[], resource: Resource, response: AdmissionResponse) {
        const violationMessages = AdmissionController.getViolationsMessages(violations, resource);
    ...
    private static getViolationsMessages(violations: Violation[], resource: Resource): string[] {
    ...
    private static formatViolationMessage(violation: Violation, resource: Resource) {
