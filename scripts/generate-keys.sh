...
# The certificate will be issued for the Common Name (CN) of `webhook-server.monokle-admission-controller.svc`, which is the
...
CN = webhook-server.monokle-admission-controller.svc
[ v3_req ]
...
[alt_names]
DNS.1 = webhook-server.monokle-admission-controller.svc
EOF
...
openssl req -new -key webhook-server-tls.key -subj "/CN=webhook-server.monokle-admission-controller.svc" -config server.conf \
